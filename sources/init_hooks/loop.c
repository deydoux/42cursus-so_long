...
/*   Updated: 2024/03/14 16:39:14 by deydoux          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	loop(t_game *game)
{
	...
	if (update)
	{
		...
		mlx_put_image_to_window(game->mlx, game->win.ptr, game->map.img.ptr, x,
			y);
		put_img_to_win(game->sprites.player_down[game->sprites.alt], *game, game->win.width / 2 + IMAGE_SIZE / 2, game->win.heigh / 2 + IMAGE_SIZE / 2);
