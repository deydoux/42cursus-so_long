...
/*   Updated: 2024/03/24 21:06:12 by deydoux          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_move(t_game *game)
{
	...
	if (game->map.str[(game->pos.x + move.x + IMAGE_SIZE * (move.x > 0)) / IMAGE_SIZE + game->pos.y / IMAGE_SIZE * (game->map.width + 1)] == '1'
		|| game->map.str[(game->pos.x + move.x + IMAGE_SIZE * (move.x > 0)) / IMAGE_SIZE + (game->pos.y + IMAGE_SIZE - 1) / IMAGE_SIZE * (game->map.width + 1)] == '1')
		move.x = 0;
	game->pos.x += move.x;
	if (game->map.str[game->pos.x / IMAGE_SIZE + (game->pos.y + move.y + IMAGE_SIZE * (move.y > 0)) / IMAGE_SIZE * (game->map.width + 1)] == '1'
		|| game->map.str[(game->pos.x + IMAGE_SIZE - 1) / IMAGE_SIZE + (game->pos.y + move.y + IMAGE_SIZE * (move.y > 0)) / IMAGE_SIZE * (game->map.width + 1)] == '1')
		move.y = 0;
	game->pos.y += move.y;
}
